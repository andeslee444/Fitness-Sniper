...
export class MarianaTekAdapter {
  ...
  /**
   * Get iframe with schedule content
   */
  async getScheduleFrame(): Promise<Frame | null> {
  ...
  async reserveClass(location: string, time: string): Promise<{ success: boolean; message: string }> {
    ...
    // Wait for spot selection modal
    await frame.waitForSelector('button:has-text("Available Spot")', { timeout: 10000 });
    
    // Select first available spot
    const spotButton = await frame.$('button:has-text("Available Spot")');
    if (spotButton) {
...
export async function testAarmyLogin() {
  ...
  await adapter.init();
  const loggedIn = await adapter.login();
  console.log('Aarmy login:', loggedIn ? 'SUCCESS' : 'FAILED');

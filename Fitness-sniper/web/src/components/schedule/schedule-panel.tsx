...
interface SchedulePanelProps {
  /**
   * When provided, clicking "Snipe This" on a class calls this callback instead
   * of the inline Popover snipe. Allows parent components (e.g. SnipeConfigSheet)
   * to intercept the click and open a full configuration form.
   */
  onSnipeClick?: (cls: ScheduleClass, studioSlug: string, locationId: string) => void;
}

export function SchedulePanel({ onSnipeClick }: SchedulePanelProps) {
  ...
  // Inline snipe state (used when onSnipeClick is NOT provided)
  ...
  // Next available handler — searches next 14 days for the earliest open slot (SCHED-03)
  ...
  // Inline snipe handler — used when onSnipeClick is NOT provided
          ...
          {classes.map((cls, i) => {
            ...
            return (
              ...
              >
                ...
                {canSnipe && (
                  onSnipeClick ? (
                    // Callback mode: parent intercepts click (used by SnipeConfigSheet in Plan 04-02)
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    // Inline Popover mode: quick one-time snipe (fallback when no parent handler)
                    <Popover

...
export async function sendBookingEmail(notification: BookingNotification): Promise<void> {
  const { to, success } = notification;
  const studioName = escapeHtml(notification.studioName);
  const classTime = escapeHtml(notification.classTime);
  const classDate = escapeHtml(notification.classDate);
  const location = escapeHtml(notification.location);
  const spot = notification.spot ? escapeHtml(notification.spot) : null;
  const message = notification.message ? escapeHtml(notification.message) : '';
  ...
  try {
    await getResend().emails.send({
      from: 'Fitness Sniper <noreply@fitnesssniper.app>',
      to: [to],
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
          ...
          <p style="margin: 0; color: #999; font-size: 12px;">Fitness Sniper</p>
        </div>
      `,
    });
  } catch (err) {
